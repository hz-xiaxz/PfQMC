...
public:
  ...
  // ===== Replica support =====
  const int
      nReplicas; // Number of replicas (default 1 for backward compatibility)
  ...
  SpinlessVOperator(const SpinlessTvUtils *_config, std::vector<iVecType *> _s,
                    int _bondType, rdGenerator *_rd, int _nReplicas = 1)
      : config(_config), etaM(_config->etaM), bondType(_bondType),
        nReplicas(_nReplicas), nDimSingle(_config->nDim),
        nDim(_nReplicas * _config->nDim), singleMaj(_config->singleMaj),
  ...
  SpinlessVOperator(const SpinlessTvUtils *_config, iVecType *_s, int _bondType,
                    rdGenerator *_rd)
      : SpinlessVOperator(_config, std::vector<iVecType *>{_s}, _bondType, _rd,
                          1) {}
  ...
  void rebuildFullB() {
    ...
    for (int r = 0; r < nReplicas; r++) {
      B.block(r * nDimSingle, r * nDimSingle, nDimSingle, nDimSingle) =
          B_replica[r];
    }
  }
  ...
  void reCalcInv() {
    ...
    for (int r = 0; r < nReplicas; r++) {
      ...
      B_inv.block(r * nDimSingle, r * nDimSingle, nDimSingle, nDimSingle) =
          B_inv_r;
    }
  }
  ...
  DataType getRatio(MatType &g, int replica, int idxAux) {
    ...
    if (hsScheme == 0) {
      tmp[0] = (1.0 -
                ((1.0i) * (config->thlV) * double(auxCur) * g(idx1_g, idx2_g)));
      tmp[1] = (1.0 -
                ((1.0i) * (config->thlV) * double(auxCur) * g(idx3_g, idx4_g)));
      r = tmp[0] * tmp[1];
      r += (config->thlV * config->thlV) *
           ((g(idx1_g, idx3_g) * g(idx2_g, idx4_g)) -
            (g(idx2_g, idx3_g) * g(idx1_g, idx4_g)));
      r *= etaM;
    } else { // hsScheme == 1
      tmp[0] = (1.0 -
                ((1.0i) * (config->thlV) * double(auxCur) * g(idx1_g, idx3_g)));
      tmp[1] = (1.0 +
                ((1.0i) * (config->thlV) * double(auxCur) * g(idx2_g, idx4_g)));
      r = tmp[0] * tmp[1];
      r -= (config->thlV * config->thlV) *
           ((g(idx1_g, idx2_g) * g(idx3_g, idx4_g)) -
  ...
  // Update a single replica within a block pair (optimized for block-diagonal
  // structure)
  void updateReplicaInBlock(MatType &g, int replica, int idxAux) {
    ...
    if (hsScheme == 0) {
      for (int imaj = 0; imaj < 2; imaj++) {
        ...
        DataType tmp_update = (1.0 - ((1.0i) * (config->thlV) * double(auxCur) *
                                      g(idx1_g, idx2_g)));
        ...
        zgeru(&blkSize, &blkSize, &alpha, x1_seg.data(), &inc, x2_seg.data(),
              &inc, g_block_ptr, &nDim);

        alpha = -alpha;
        zgeru(&blkSize, &blkSize, &alpha, x2_seg.data(), &inc, x1_seg.data(),
              &inc, g_block_ptr, &nDim);
      }
    } else { // hsScheme == 1
      ...
      for (int iaux = 0; iaux < 2; iaux++) {
        ...
        if (iaux == 0) {
          idx1 = idxj1;
          idx2 = idxj2;
          tmp_update = (1.0 - ((1.0i) * (config->thlV) * double(auxCur) *
                               g(offset + idx1, offset + idx2)));
        } else {
          idx1 = idxk1;
          idx2 = idxk2;
          tmp_update = (1.0 + ((1.0i) * (config->thlV) * double(auxCur) *
  ...
  void updateSingleReplica(MatType &g, int replica, int idxAux) {
    ...
    int auxCur = (*s[replica])(idxAux);
    const int inc = 1;
    // For single replica, we update the whole matrix (or the relevant block if
    // we had block diagonal structure, but here nReplicas=1 implies nDim =
    // nDimSingle)
    int nDim_local = nDim;
    ...
    if (hsScheme == 0) {
      for (int imaj = 0; imaj < 2; imaj++) {
        ...
        zgeru(&nDim_local, &nDim_local, &alpha, x1.data(), &inc, x2.data(),
              &inc, g.data(), &nDim_local);

        alpha = -alpha;
        zgeru(&nDim_local, &nDim_local, &alpha, x2.data(), &inc, x1.data(),
              &inc, g.data(), &nDim_local);
      }
    } else { // hsScheme == 1
      int idxj1, idxk1, idxj2, idxk2;
  ...
  void updateReplicaInBlockSingleMajorana(MatType &g, int replica, int idxAux) {
    int p = replica / 2;
    int blkOffset = p * 2 * nDimSingle;
    int blkSize = 2 * nDimSingle;
    DataType *g_block_ptr = g.data() + blkOffset + blkOffset * nDim;
    ...
    // Update global B
    if (nReplicas == 1) {
      B(idx1, idx2) = B_replica[replica](idx1, idx2);
      B(idx2, idx1) = B_replica[replica](idx2, idx1);
    } else {
      int offset = replica * nDimSingle;
      B(offset + idx1, offset + idx2) = B_replica[replica](idx1, idx2);
      B(offset + idx2, offset + idx1) = B_replica[replica](idx2, idx1);
    }
    ...
    zgeru(&blkSize, &blkSize, &alpha, x1_seg.data(), &inc, x2_seg.data(), &inc,
          g_block_ptr, &nDim_local);

    alpha = -alpha;
    zgeru(&blkSize, &blkSize, &alpha, x2_seg.data(), &inc, x1_seg.data(), &inc,
          g_block_ptr, &nDim_local);
  }

  void singleFlipSingleMajorana(MatType &g, int idxAux, bool &flag,
                                DataType &signCur) {
    // Loop over all replicas with independent acceptance
    for (int replica = 0; replica < nReplicas; replica++) {
      double rand = rd->rdUniform01();
      DataType r = getRatioSingleMajorana(g, replica, idxAux);
      total++;
      if (rand < std::abs(r)) {
        accepted++;
        flag = true;
        signCur *= (r / std::abs(r));
        updateReplicaInBlockSingleMajorana(g, replica, idxAux);
      }
    }
  }
  ...
  // Return the first replica's aux field for backward compatibility with
  // single-replica code/tests
